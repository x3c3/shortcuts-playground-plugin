...
55. **Automation triggers are metadata-only.** `AUTOMATION_TRIGGERS.md` and `data/toolkit-v78-trigger-parameter-keys.json` document OS 27 ToolKit trigger IDs, Python names, parameter keys, and output types. They do not prove importable Personal Automation or inline trigger serialization. Do not create automation trigger plists or Shortcuts database rows from this metadata until an exported automation/shortcut sample verifies the top-level payload.
