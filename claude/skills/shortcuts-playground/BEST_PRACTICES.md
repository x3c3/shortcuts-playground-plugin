...
- Avoid generating **Unknown Action** blocks. AppIntent/action identifiers must exist in the bundled ToolKit snapshots (`data/toolkit-v*-tool-ids.json`) and references (`APPINTENTS.md`, `ACTIONS.md`); static `data/macos27-shortpy-grounding.json` can add macOS 27 schema confidence but does not override target availability. Local ToolKit DB checks are optional and additive for audits only. Do not invent identifiers. If an action is unavailable on the target OS/version, replace it with a supported fallback and note it in a Comment.
