...
Added HealthKit support from Federico's local iPhone exports, local Xcode iPhoneOS 26.2 HealthKit headers, and local ActionKit unit constants. The skill now documents and validates Find Health Samples, Get Details of Health Sample, Log Health Sample, and Log Workout.

### Fixes Applied

- Added `HEALTHKIT.md` with local XML evidence for HealthSnap, Log Health Sample quantity/category samples, Get Details of Health Sample, and Log Workout identifier coverage.
